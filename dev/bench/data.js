    ...
    "Benchmark": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "67644727cc5e20ba9053cb5e42fe9f7de897fba3",
          "message": "Fix has_escape scanner bug: non-quote escapes (\\n, \\t, \\r, \\uXXXX) were invisible to the tape scanner\n\nWhile chasing \"next round\" transform-pipeline optimizations, found a serious\npre-existing correctness bug: the tape scanner's has_escape detection (used\nto decide whether unescape_json_string needs to run before auto_convert_types,\nvalue_replacement, key_replacement, lowercase_keys, or collision handling can\nsee a string's real content) only detected escaped quotes (\\\") and backslash\nruns immediately preceding a matched quote character. It worked by memchr'ing\nstraight to the next `\"` and counting backslashes right before it -- any\nescape sequence NOT adjacent to a quote (a lone \\n, \\t, \\r, \\b, \\f, \\/, \\uXXXX,\nor even a lone \\\\ in the middle of a string) was completely invisible to it.\n\nConcretely: `.auto_convert_types(true)` on `\"123\\t\"` silently failed to\nrecognize it as a number, because has_escape was false, so\nunescape_json_string was never called, so the string being checked was the\nstill-escaped literal text \"123\\t\" (backslash + t), not \"123\" + a real tab\ncharacter trimmed away. Any transform reading a key or value containing only\nnon-quote escapes was silently operating on the wrong (still-escaped) text.\n\nRoot cause: a JSON string can only contain a backslash as part of a valid\nescape sequence, so \"any backslash in the content\" is exactly equivalent to\n\"needs unescaping\" -- the fix replaces the backslash-before-quote heuristic\nwith a direct memchr(b'\\\\', content) over the full string range once the\nreal closing quote is found. Still SIMD-fast (one extra memchr per string),\nnow actually correct.\n\nAlso fixed, in the same area: value_replacement + auto_convert_types used to\nunescape the same string twice (once per feature's own check) whenever the\nreplacement pattern didn't match -- now unescapes once and reuses the Cow for\nboth checks, in both DirectWalker and CollectingWalker.\n\nAdded regression tests: test_scanner_detects_non_quote_escapes exercises\n\\t/\\n/\\r/\\uXXXX in isolation via auto_convert_types and a literal\nvalue_replacement; test_escaped_value_falls_through_replacement_to_auto_convert\ncovers the double-unescape fix on both the Direct and Collecting walker paths.\n\nVerified: full test suite (all features), stress benchmark suite\n(correctness mode), clippy/fmt clean, all three examples.\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T17:16:39+10:00",
          "tree_id": "3e2af13429c4e4bfb3afce9d707b469b500dda8d",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/67644727cc5e20ba9053cb5e42fe9f7de897fba3"
        },
        "date": 1783930244895,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "3d83bfb55bea97730c0cc6f0d5a1d354f0b54a52",
          "message": "Implement r'...' regex syntax; bare patterns are now literal (BREAKING)\n\nFound while investigating the transform pipeline: the docs and README\npromised a `regex:` prefix to opt into regex matching for key_replacement/\nvalue_replacement (\"Literal and regex-based key/value replacements...\nPrefix patterns with `regex:` to use regular expressions\"). That prefix was\nnever implemented anywhere in the code -- grep for it across src/ turned up\nnothing. The actual behavior was \"always compile as regex; fall back to\nliteral only if the pattern fails to compile as valid regex syntax\", which\nmeant:\n  - Every documented `regex:`-prefixed example silently never matched\n    anything (the literal text \"regex:\" became part of the compiled\n    pattern, which can then never match real data) -- including in this\n    project's own combo_max_all_features and realworld_benchmarks.rs\n    benchmarks, which have been measuring failed-match attempts, not real\n    replacement work, for as long as that convention has been in the docs.\n  - There was no way to match a pattern containing regex metacharacters\n    (\".\", \"$\", \"(\", etc.) literally -- the \"Literal... replacements\" half\n    of the advertised feature didn't really exist as a distinct mode.\n\nNew syntax: wrap a pattern in `r'...'` (e.g. `r'^admin_'`) to use it as\nregex; any other pattern is matched as a literal, exact substring --\nincluding ones with characters that would otherwise be regex\nmetacharacters. A malformed r'...' pattern is treated as \"no match\" rather\nthan erroring, since there's no config-time validation point for\nreplacement patterns (patterns are stored as raw strings and only resolved\nduring execute()).\n\nImplementation: cache.rs gains parse_pattern()/ParsedPattern (the r'...'\nparser, reusing get_cached_regex's cache keyed on the *inner* pattern\ntext), used by both apply_key_replacement_patterns (transform.rs) and\napply_value_replacement_cow (flatten.rs) -- the two places pattern\nmatching actually happens.\n\nThis is a real breaking change for anyone whose bare (non-prefixed)\npatterns relied on implicit regex behavior -- see CHANGELOG.md. Updated\nevery call site across the repo accordingly: ~50 benchmark/example\npatterns converted from the non-functional `regex:` prefix, ~15 Rust unit\ntests and ~15 Python tests that relied on bare-pattern regex behavior now\nuse r'...', plus doctests in lib.rs/builder.rs, the mdBook guide pages, and\nREADME. Added new tests covering: bare patterns with regex metacharacters\nstaying literal, r'...' patterns behaving as regex, malformed r'...'\npatterns being silently ignored, and edge cases (too-short-to-be-wrapped\ntext, empty r'' regex).\n\nVerified end-to-end, not just unit tests: examples/advance_usage.rs's key\nprefix-stripping step (api_response::, user_data::, etc.) now visibly\nworks in its own output, where before the prefixes silently remained.\n\nVerified: full Rust test suite (all features) + doctests, full Python test\nsuite (150 tests) against a real maturin-built extension, all 5 Criterion\nbenchmark suites in correctness-check mode, clippy/fmt clean.\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T20:50:00+10:00",
          "tree_id": "0466f20f8784f54e152b9af1c47fa5fde04ba00a",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/3d83bfb55bea97730c0cc6f0d5a1d354f0b54a52"
        },
        "date": 1783942741839,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "01f9b195de84e9c6e04389d79f542c5f6889f51a",
          "message": "Add JVM (Java) bindings via JNI, for use as Spark UDFs on Databricks\n\nFull feature parity with the Python bindings (regex/literal key & value\nreplacement, filtering, casing, type conversion) via a new opt-in `jvm`\nCargo feature (src/jvm.rs) and a Maven project (jvm/) with two usage\ntiers: a simple SQL-callable row UDF and a higher-throughput batched\nmapPartitions transform. New jvm-ci.yml builds and tests a multi-platform\n(linux-x86_64, linux-aarch64) fat jar.\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T23:09:45+10:00",
          "tree_id": "5da2e311e919fbbdb0c471c97e0cc79fcfbec982",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/01f9b195de84e9c6e04389d79f542c5f6889f51a"
        },
        "date": 1783951113021,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "f683b2efeeb77be94c1c73c647f5055c384f8d52",
          "message": "Document JVM/Spark bindings; fix stale bare-regex examples in docs\n\nAdd a Quick Start (JVM / Spark) page plus installation/introduction/\narchitecture updates for the JNI bindings shipped last commit. Also\nfixes several doc examples (quickstart-python, normal-mode, rust-api,\npython-api, troubleshooting) that predated the r'...' regex syntax\nchange and were showing now-incorrect behavior for bare patterns like\n\"^user_\" (now literal, not regex).\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-14T06:18:29+10:00",
          "tree_id": "b5c698b72bac051a046031623ea91dc289ef0939",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/f683b2efeeb77be94c1c73c647f5055c384f8d52"
        },
        "date": 1783976802518,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "e6024449a70aae17b9d9d33b1bbc7faa0c058f22",
          "message": "Add Maven Central publishing for the JVM bindings on tagged releases\n\nNew `release` Maven profile in jvm/pom.xml (sources/javadoc jars, GPG\nsigning, Sonatype's central-publishing-maven-plugin) stays fully opt-in\nbehind `-P release` so normal `mvn test`/`mvn package` never needs a\nGPG key. New `release` job in jvm-ci.yml triggers only on a tag push\n(deliberately narrower than the PyPI release job -- a Central publish\nis permanent) and is gated behind the same Cargo.toml/pom.xml\nversion-sync check the `package` job already runs.\n\nCredentials (Central Portal token, a freshly generated GPG signing\nkey) added as repo secrets out of band.\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-14T07:18:58+10:00",
          "tree_id": "4ec9b7febb33c1bb4544bb59b9790f9f0d0a41d1",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/e6024449a70aae17b9d9d33b1bbc7faa0c058f22"
        },
        "date": 1783980429399,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "1b50b722ac849ddd88ad66fba438081ddcd17310",
          "message": "Enable crates.io publishing on tagged releases\n\nRemoves publish = false and activates the previously-dormant,\ncommented-out cargo publish step in maturin-ci.yml's tag-gated release\njob (it already had the right guard: only runs on an actual tag push).\nAlso trims the published package to Rust-relevant files -- jvm/,\npython/, docs/, and tooling config were all getting bundled into the\ncrate archive by default now that this repo hosts multiple language\nbindings.\n\nRotated CARGO_REGISTRY_TOKEN with a fresh crates.io token out of band.\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-14T12:07:26+10:00",
          "tree_id": "35938a08b81e7986c6f239c69f303593ad4e5485",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/1b50b722ac849ddd88ad66fba438081ddcd17310"
        },
        "date": 1783997751507,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "cba0d37b4f20b5d2625bdf20a451b629d1f917c1",
          "message": "Correct JVM bindings docs: Lakeflow Pipelines don't support JVM libraries at all\n\nDatabricks' own docs are explicit that pipelines support only SQL and\nPython and that JVM libraries cannot be attached to pipeline compute\n(serverless or classic-backed) -- confirmed directly against their\ndocs, not assumed. Everything written earlier describing\nspark.udf.registerJavaFunction usage \"inside a Lakeflow pipeline\" was\nwrong: the jar can never be attached to a pipeline's compute, so the\nclass would never be on the classpath to register.\n\nCorrects jvm/README.md, quickstart-jvm.md, introduction.md, and the\nearlier CHANGELOG entry to describe what actually works: the JVM\nbindings for Databricks Jobs/notebooks on classic compute (fully\nsupported there), and a two-stage Job-then-pipeline pattern for\nfeeding a Lakeflow pipeline from JVM-processed output.\n\nAdds docs/src/guide/databricks-setup.md with the solution for running\ngenuinely inside a pipeline: the Python bindings wrapped in a\npandas_udf, since Python packages (unlike JVM jars) are a supported\npipeline dependency. This gives the same per-batch-not-per-row\nthroughput as the JVM bindings' BatchTransform tier, runs distributed\nacross executors (not collected to the driver), and was validated\nagainst a real local Spark session before being documented.\n\nCo-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-14T17:50:57+10:00",
          "tree_id": "c1b7775617a1f31fbe4dffb48edf76f1f67df272",
          "url": "https://github.com/amaye15/JSON-Tools-rs/commit/cba0d37b4f20b5d2625bdf20a451b629d1f917c1"
        },
        "date": 1784018366742,
        "tool": "cargo",
        "benches": [
