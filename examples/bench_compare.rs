//! Compares two commits' worth of rows in `benches/history.csv`, joining on everything
//! except `time_us` so it never compares across different targets/thread-counts by accident.
//!
//! ```bash
...
fn parse_csv(path: &std::path::Path) -> Vec<Row> {
    ...
    for line in content.lines() {
        // Header line(s) — CI concatenates one CSV output per OS/arch leg, each with its
        // own header, so more than one can appear.
        ...
        let (Ok(threads), Ok(time_us)) = (f[4].parse::<u32>(), f[8].parse::<f64>()) else {
            eprintln!("skipping malformed row: {line}");
            continue;
        };
        out.push(Row {
            ...
            key: Key {
                ...
                arch: f[3].to_string(),
                threads,
                operation: f[5].to_string(),
                scenario: f[6].to_string(),
                size: f[7].to_string(),
            },
            time_us,
        });
...
fn main() {
    ...
    // Distinct commits in the order they first appear — since new measurements are
    // always appended, this is chronological.
    ...
    println!(
        "{:<10} {:>5} {:<14} {:<18} {:>6} | {:>10} {:>10} {:>8}",
        "target", "thr", "operation", "scenario", "size", "old_us", "new_us", "change"
    );
    ...
    for key in &keys {
        ...
        match (old_map.get(key), new_map.get(key)) {
            (Some(&old_t), Some(&new_t)) => {
                ...
                println!(
                    "{target:<10} {:>5} {:<14} {:<18} {:>6} | {old_t:>10.3} {new_t:>10.3} {pct:>+7.1}% {marker}",
                    key.threads, key.operation, key.scenario, key.size,
                );
            }
            ...
            (None, Some(&new_t)) => {
                only_new.push(key);
                println!(
                    "{target:<10} {:>5} {:<14} {:<18} {:>6} | {:>10} {new_t:>10.3} {:>9}",
                    key.threads, key.operation, key.scenario, key.size, "-", "new",
                );
            }
    ...
    if !only_old.is_empty() {
        println!(
            "\n{} measurement(s) present only in {old_commit}, not in {new_commit} (target/scenario not re-measured)",
            only_old.len()
        );
    }
    if !only_new.is_empty() {
        println!(
            "{} measurement(s) present only in {new_commit}, not in {old_commit} (new target/scenario, printed above as \"new\")",
            only_new.len()
        );
    }
}
