...
fn custom_separator_example() -> Result<(), Box<dyn Error>> {
    ...
    let result = flatten_json(json, false, false, false, false, None, None, Some("_"), false)?;
...
fn replacement_examples() -> Result<(), Box<dyn Error>> {
    ...
    let result = flatten_json(json1, false, false, false, false, key_replacements, None, None, false)?;
    ...
    let result = flatten_json(json2, false, false, false, false, None, value_replacements, None, false)?;
    ...
    let key_replacements = Some(vec![("user_".to_string(), "person_".to_string())]);
    let value_replacements = Some(vec![("@example.com".to_string(), "@company.org".to_string())]);
    let result = flatten_json(json3, false, false, false, false, key_replacements, value_replacements, None, false)?;
...
fn regex_examples() -> Result<(), Box<dyn Error>> {
    ...
    let key_replacements = Some(vec![("regex:^field_(\\d+)_(.+)".to_string(), "$2_id_$1".to_string())]);
    let result = flatten_json(json3, false, false, false, false, key_replacements, None, None, false)?;
    println!("Capture groups input:  {}", json3);
    println!("Capture groups output: {}\n", extract_single(result));
    
    // Phone number formatting
    let json4 = r#"{"phone": "+1-555-123-4567", "fax": "+1-555-987-6543"}"#;
    let value_replacements = Some(vec![("regex:\\+(\\d)-(\\d{3})-(\\d{3})-(\\d{4})".to_string(), "($2) $3-$4".to_string())]);
    let result = flatten_json(json4, false, false, false, false, None, value_replacements, None, false)?;
...
fn error_handling_example() {
    ...
    match flatten_json(invalid_json, false, false, false, false, None, None, None, false) {
    ...
    match flatten_json(r#"{"test": "value"}"#, false, false, false, false, key_replacements, None, None, false) {
        Ok(_) => println!("Unexpected success"),
        Err(e) => {
            if let Some(flatten_err) = e.downcast_ref::<FlattenError>() {
                match flatten_err {
...
fn real_world_example() -> Result<(), Box<dyn Error>> {
    ...
    // Basic flattening
    let result = flatten_json(product_json, false, false, false, false, None, None, None, false)?;
    println!("\nBasic flattening:");
    ...
    let result = flatten_json(product_json, false, true, true, true, key_replacements, None, None, false)?;
