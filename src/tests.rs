...
use crate::{JsonFlattener, JsonUnflattener, JsonInput, JsonOutput};
...
/// This can be used to adjust test expectations for CI environment
#[cfg(test)]
...
mod integration_tests {
    ...
    fn test_real_json_remove_empty_strings() {
        ...
        let with_empty_count =
            count_keys(&extract_single(result_with_empty.unwrap()));
        let without_empty_count =
            count_keys(&extract_single(result_without_empty.unwrap()));
...
// ===== COMPREHENSIVE EDGE CASE TESTS =====
    ...
    fn test_edge_case_special_character_values() {
        ...
        assert_eq!(parsed["url"], "https://example.com/path?param=value&other=123");
    ...
    fn test_edge_case_json_string_values() {
        ...
        assert_eq!(parsed["json_string_nested"], "{\"outer\": {\"inner\": \"value\"}}");
        assert_eq!(parsed["json_string_with_escapes"], "{\"escaped\": \"value\\nwith\\tescapes\"}");
        assert_eq!(parsed["not_json_but_looks_like"], "{this is not valid json}");
    ...
    fn test_edge_case_filtering_with_nested_empty_values() {
        ...
        assert!(!parsed.as_object().unwrap().contains_key("level1.remove_empty_string"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.remove_null"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.remove_empty_object"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.remove_empty_array"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.level2.also_remove_empty"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.level2.also_remove_null"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.level2.nested_empty_object"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.level2.nested_empty_array"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.level2.level3.deep_empty"));
        assert!(!parsed.as_object().unwrap().contains_key("level1.level2.level3.deep_null"));
    ...
    fn test_edge_case_replacement_with_empty_strings() {
        ...
        assert!(!parsed.as_object().unwrap().contains_key("replace_with_empty"));
    ...
    fn test_edge_case_extreme_separator_configurations() {
        ...
        for (separator, expected_key) in separators {
            ...
            assert_eq!(parsed[expected_key], "value",
    ...
    fn test_edge_case_performance_with_pathological_input() {
        ...
        // Wide structure
        for i in 0..100 {
            json_obj.insert(format!("wide_key_{}", i), serde_json::Value::String("value".to_string()));
        }
        ...
        let large_array: Vec<serde_json::Value> = (0..100)
            ...
            .collect();
        json_obj.insert("large_array".to_string(), serde_json::Value::Array(large_array));
        ...
        assert!(key_count > 200, "Should have processed many keys: {}", key_count);
    }
    ...
    fn test_edge_case_complex_regex_patterns() {
        ...
        let result = JsonFlattener::new()
            ...
            .value_replacement("regex:\\+?1?[-.]?(\\d{3})[-.]?(\\d{3})[-.]?(\\d{4})", "($1) $2-$3")
    ...
    fn test_edge_case_zero_length_strings_and_arrays() {
        ...
        assert!(parsed1.as_object().unwrap().contains_key("zero_length_array"));
        assert_eq!(parsed1["nested.another_empty"], "");
        assert!(parsed1.as_object().unwrap().contains_key("nested.another_array"));
    ...
    fn test_edge_case_concurrent_modification_simulation() {
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            assert_eq!(parsed, first_parsed, "Result {} differs from first result", i);
        }
    }
    ...
    fn test_mixed_object_array_unflattening() {
        let flattened = r#"{"users.0.name": "John", "users.0.age": 30, "users.1.name": "Jane", "users.1.age": 25}"#;
        let result = JsonUnflattener::new().unflatten(flattened).unwrap();
        let unflattened = extract_single(result);
        let parsed: Value = serde_json::from_str(&unflattened).unwrap();
    ...
    fn test_unflattener_with_regex_replacements() {
        let flattened = r#"{"user_name": "John", "user_email": "john@example.com", "admin_role": "super"}"#;
        let result = JsonUnflattener::new()
    ...
    fn test_unflattener_multiple_inputs() {
        ...
        let result = JsonUnflattener::new().unflatten(flattened_list.as_slice()).unwrap();
    ...
    fn test_unflattener_very_deep_nesting() {
        ...
        assert_eq!(parsed["a"]["b"]["c"]["d"]["e"]["f"]["g"]["h"]["i"]["j"]["k"]["l"]["m"]["n"]["o"]["p"]["q"]["r"]["s"]["t"]["u"]["v"]["w"]["x"]["y"]["z"], "deep_value");
    }
    ...
    fn test_unflattener_numeric_string_keys() {
        ...
        let flattened = r#"{"items.01": "leading_zero", "items.001": "more_zeros", "items.0": "valid_index"}"#;
        let result = JsonUnflattener::new().unflatten(flattened).unwrap();
        let unflattened = extract_single(result);
        let parsed: Value = serde_json::from_str(&unflattened).unwrap();
    ...
    fn test_unflattener_mixed_separators_as_keys() {
        ...
        let flattened = r#"{"user.name_first": "John", "data.array_0": "value", "config.setting_enabled": true}"#;
        let result = JsonUnflattener::new()
    ...
    fn test_unflattener_multiple_separator_patterns() {
        ...
        for (separator, flattened) in test_cases {
            ...
            assert_eq!(parsed["user"]["profile"]["name"], "John", "Failed for separator: {}", separator);
        }
    }
    ...
    fn test_unflattener_concurrent_safety_simulation() {
        ...
        let unflattener = JsonUnflattener::new()
            .separator(".")
            .lowercase_keys(false);

        let test_cases = [r#"{"user.name": "John"}"#,
